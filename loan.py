...
class LoanCalculator:
	...
	def __init__(self):
		...
		Label(window, text=None).grid(row=4,column=1, sticky = W) # space between inputs and outputs
		...
		Entry(window, textvariable = self.pv, justify=RIGHT).grid(row=1,column=2)
		...
		Label(window, textvariable = self.pmt, 
			font="Helvetica 12 bold", justify=RIGHT).grid(row=5,column=2,sticky= E )
		Label(window, textvariable = self.total, 
			font="Helvetica 12 bold", justify=RIGHT).grid(row=6,column=2, sticky= E)


		Button(window, text="Calculate Payment", command=self.calcPayment).grid(row=7, column=1,sticky=W)
